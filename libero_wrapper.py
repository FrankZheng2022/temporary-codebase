...
class LiberoWrapper(gym.Wrapper):
    def __init__(self, env, img_size, frame_stack, seed):
        super().__init__(env)
        self.env = env
        env.seed(seed)
        self._num_frames = frame_stack
        self._frames_agentview = deque([], maxlen=self._num_frames)
        self._frames_wristview = deque([], maxlen=self._num_frames)
    ...
    def reset(self):
        ...
        for _ in range(self._num_frames):
            self._frames_agentview.append(self._get_pixel_obs(state['agentview_image']))
        obs['agentview'] = np.concatenate(list(self._frames_agentview), axis=0) 
        for _ in range(self._num_frames):
            self._frames_wristview.append(self._get_pixel_obs(state['robot0_eye_in_hand_image']))
        obs['wristview'] = np.concatenate(list(self._frames_wristview), axis=0)
...
def make(task_id, task_suite_name, seed=1, frame_stack=1, train=False, img_size=128, episode_length=500, device_id=-1):
    ...
    env = LiberoWrapper(env, img_size, frame_stack, seed)
