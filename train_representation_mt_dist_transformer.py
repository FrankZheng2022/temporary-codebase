...
class Workspace:
    def __init__(self, cfg, rank, world_size):
        ...
        self.pretraining_data_dirs = []
        if self.cfg.stage < 3:
            for task_id in range(90): ### libero_90
        ...
        if self.cfg.stage < 3:
            ...
            self.eval_dir = self.results_dir / 'eval' / f'stage_{self.cfg.stage}' / task_name
        self.eval_dir.mkdir(parents=True, exist_ok=True)

        if self.cfg.stage == 2:
            return
        self.setup_replay_buffer()
        # obs_agent, obs_wrist, state, action, action_seq, next_obs_lst = next(self.replay_iter)
        # print(obs_agent.shape, obs_wrist.shape, state.shape, action.shape, action_seq[0].shape, next_obs_lst[0][0].shape)
        # raise Exception
        ...
        while self.global_step < self.cfg.num_train_steps:
            if self.global_step%self.cfg.eval_freq == 0 and self.rank == 0:
                ...
                if metrics is not None:
                    # log stats
                    print('SPR_LOSS:{}, QUANTIZE_LOSS:{}, DECODER_LOSS:{}'.format(metrics['spr_loss'], metrics['quantize_loss'], metrics['decoder_loss']))
